...
L1 distance was used because it's the assignment's own grading metric, and it's also less sensitive to outlier points than a squared (L2) error would be. Finding the nearest curve point for all 1500 data points on every single optimizer iteration would be far too slow with brute-force comparison, so a `cKDTree` was used instead, turning each lookup into a fast operation rather than a full scan. For the search itself, `differential_evolution` was chosen over a simple local search because it explores the entire bounded parameter space with a population of candidates rather than improving from one starting guess — important here since the periodic `sin(0.3t)` term makes the loss landscape non-convex, and a purely local method can converge confidently on a wrong answer that only looks like a minimum. A coarser curve sampling (1500 points) was used during this global stage purely for speed, since the objective gets evaluated thousands of times, while a much finer sampling (20,000 points) was reserved for the final polish, where only a handful of evaluations are needed. That final polish used `Nelder-Mead` rather than a gradient-based method, since the nearest-neighbor lookup makes the loss function technically non-smooth at a fine scale — small parameter changes can flip which curve point counts as "closest" — which gradient-based optimizers don't handle well.
...
This works because θ and X can already be undone exactly with linear algebra for any candidate, so there's no reason to spend optimizer iterations rediscovering something that can be computed directly — that's the whole motivation for a second method. Turning the exponential relationship into a straight line via a logarithm means solving for M becomes a simple regression instead of another nonlinear search, which is both faster and more numerically reliable. Points near the zero-crossings of `sin(0.3t)` were excluded before this regression, since dividing by a value close to zero amplifies any small numerical noise and would distort the fit; points where the resulting ratio came out negative or near zero were dropped too, since `e^(M|t|)` must always be positive, so a bad ratio signals either a wrong `(θ, X)` guess or an unreliable point. The regression was also forced through the origin, matching the fact that the true model has no constant offset term at `t = 0`. Even with M eliminated, the remaining 2D search over θ and X is still non-convex for the same periodic reason as before — an early attempt starting `Nelder-Mead` from an arbitrary point converged on a clearly wrong answer (θ ≈ 14°, high residual) — so a coarse grid scan across the full bounded region was run first to find the correct basin before refining locally.
